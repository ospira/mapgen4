  ...
  for (let i = 0; i < queue_t.length; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      if (t_distance[neighbor_t] === -1) {
        let increment =
          spacing * (1 + jaggedness * (randFloat() - randFloat()));
...
export default class Map {
  ...
  assignTriangleElevation(elevationParam, constraints) {
    ...
    for (let t = 0; t < numSolidTriangles; t++) {
      ...
      t_elevation[t] =
        ...
        elevationParam.noisy_coastlines *
          (1 - e * e * e * e) *
          (precomputed.t_noise4[t] +
            precomputed.t_noise5[t] / 2 +
            precomputed.t_noise6[t] / 4);
    }
    ...
    for (let t = 0; t < numTriangles; t++) {
      ...
      if (e > 0) {
        ...
        let eh =
          (1 + noisiness * t_noise4[t] + (1 - noisiness) * t_noise2[t]) *
          elevationParam.hill_height;
        if (eh < 0.01) {
          eh = 0.01;
        }
        let em =
          1 - (mountain_slope / mountain_sharpness) * t_mountain_distance[t];
        if (em < 0.01) {
          em = 0.01;
        }
        let weight = e * e;
  ...
  assignElevation(elevationParam, constraints) {
    ...
    if (this.seed !== elevationParam.seed) {
      ...
      this.seed = elevationParam.seed;
      this.precomputed = precalculateNoise(
        makeRandFloat(elevationParam.seed),
        this.mesh
      );
    }
  ...
  assignRainfall(biomesParam) {
    ...
    if (biomesParam.wind_angle_deg != this.windAngleDeg) {
      ...
      for (let r = 0; r < numRegions; r++) {
        wind_order_r[r] = r;
        r_wind_sort[r] =
          mesh.r_x(r) * windAngleVec[0] + mesh.r_y(r) * windAngleVec[1];
      }
    ...
    for (let r of wind_order_r) {
      ...
      if (humidity > 1.0 - r_elevation[r]) {
        let orographicRainfall =
          biomesParam.rain_shadow * (humidity - (1.0 - r_elevation[r]));
