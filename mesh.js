...
async function choosePoints() {
  let points = undefined,
    peaks_index = undefined;
  const jitter = 0.5;
  if (param.spacing === 5) {
    let buffer = await fetch("build/points-5.data").then((response) =>
      response.arrayBuffer()
    );
    let extraction = extractPoints(buffer);
    points = applyJitter(
      extraction.points,
      param.spacing * jitter * 0.5,
      makeRandFloat(param.mesh.seed)
    );
    peaks_index = extraction.peaks_index;
  } else {
    points = applyJitter(
      hexagonGrid(1.5 * param.spacing),
      param.spacing * jitter,
      makeRandFloat(param.mesh.seed)
    );
    peaks_index = chooseMountainPeaks(
      points.length,
      param.spacing,
      makeRandFloat(param.mesh.seed)
    );
...
export async function makeMesh() {
  ...
  let builder = new MeshBuilder({
    boundarySpacing: param.spacing * 1.5,
  }).addPoints(points);
  ...
  if (
    points[200][0] !== mesh.r_x(200 + mesh.numBoundaryRegions) ||
