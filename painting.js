...
class Generator {
  ...
  generate() {
    ...
    const amplitudes = Array.from({ length: 5 }, (_, octave) =>
    ...
    function fbm_noise(nx, ny) {
      ...
      for (let octave = 0; octave < amplitudes.length; octave++) {
        ...
        sum +=
          amplitudes[octave] * noise.noise2D(nx * frequency, ny * frequency);
    ...
    for (let y = 0; y < CANVAS_SIZE; y++) {
      for (let x = 0; x < CANVAS_SIZE; x++) {
        ...
        let e =
          0.5 * (fbm_noise(nx, ny) + island * (0.75 - 2 * distance * distance));
        if (e < -1.0) {
          e = -1.0;
        }
        if (e > +1.0) {
    ...
    for (let y = top; y <= bottom; y++) {
      ...
      for (let x = left; x <= right; x++) {
        ...
        if (strength > currentStroke.strength[p]) {
          currentStroke.strength[p] =
            (1 - factor) * currentStroke.strength[p] + factor * strength;
        }
        let mix =
          currentStroke.strength[p] * Math.min(1, currentStroke.time[p]);
        elevation[p] =
          (1 - mix) * currentStroke.previousElevation[p] + mix * newElevation;
      }
    }
...
let exported = {
  ...
  setElevationParam: (elevationParam) =>
    heightMap.setElevationParam(elevationParam),
...
new Draggable({
  ...
  drag(event) {
    ...
    let coords = [event.x / output.clientWidth, event.y / output.clientHeight];
