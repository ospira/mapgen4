...
function setMeshGeometry(mesh, P) {
  ...
  if (P.length !== 2 * (numRegions + numTriangles)) {
    throw "wrong size";
  }
...
function assignTextureCoordinates(spacing, numSizes, textureSize) {
  ...
  function UV(x, y) {
    return {
      xy: [x, y],
      uv: [(x + 0.5) / textureSize, (y + 0.5) / textureSize],
    };
  }

  let triangles = [[]];
  let width =
      Math.floor((textureSize - 2 * spacing) / (2 * numSizes + 3)) - spacing,
...
const riverTexturePositions = assignTextureCoordinates(
  riverTextureSpacing,
  numRiverSizes,
  riverTextureSize
);
function createRiverBitmap() {
  ...
  function lineWidth(i) {
    const spriteSize =
      riverTexturePositions[0][1][0][0].xy[0] -
      riverTexturePositions[0][0][0][0].xy[0];
  ...
  for (let row = 0; row <= numRiverSizes; row++) {
    for (let col = 0; col <= numRiverSizes; col++) {
      for (let type = 0; type < 2; type++) {
        ...
        ctx.beginPath();
        ctx.rect(
          pos[1].xy[0] - riverTextureSpacing / 2,
          pos[0].xy[1] - riverTextureSpacing / 2,
          pos[2].xy[0] - pos[1].xy[0] + riverTextureSpacing,
          pos[2].xy[1] - pos[0].xy[1] + riverTextureSpacing
        );
        // ctx.clip(); // TODO: to make this work right, the spacing needs to vary based on the river size, I think

        let center = [
          (pos[0].xy[0] + pos[1].xy[0] + pos[2].xy[0]) / 3,
